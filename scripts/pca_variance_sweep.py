...
print(f"Bloc={BLOCK}  dim_brute={X.shape[1]}  n_patchs={len(df)}  "
      f"(Granuleux={int((df.id==7).sum())}, Sableux={int((df.id==8).sum())})")
...
for k, (tr, te) in enumerate(folds):
    ...
    print(f"  fold {k}: n_test={len(te):3d}  G={int((g.id==7).sum()):3d}  "
          f"S={int((g.id==8).sum()):3d}  images={g.image.nunique()}")
    ...
    for level in LEVELS:
        ...
        for j, m in enumerate(METRICS):
            ...
            records.append({"fold": fk, "level": LEVEL_LABELS[level], "metric": m,
                             "r2": r2, "rmse": rmse, "medae": medae,
                             "alpha": ridge.alpha_, "n_components": n_comp})
...
def agg_table(metric_col):
    piv = rec.pivot_table(index="level", columns="metric", values=metric_col,
                          aggfunc=["mean", "std"])
    piv = piv.reindex(level_order)
    return piv
...
r2_mean.to_csv(OUT / "R2_mean_par_niveau.csv")
r2_std.to_csv(OUT / "R2_std_par_niveau.csv")
rmse_mean.to_csv(OUT / "RMSE_mean_par_niveau.csv")
rmse_std.to_csv(OUT / "RMSE_std_par_niveau.csv")
medae_mean.to_csv(OUT / "MedAE_mean_par_niveau.csv")
medae_std.to_csv(OUT / "MedAE_std_par_niveau.csv")
pd.DataFrame({"n_components_mean": ncomp_mean, "n_components_std": ncomp_std}).to_csv(OUT / "n_components_par_niveau.csv")

print("\n=== R2 moyen (5 folds) -- niveau x metrique ===")
print(r2_mean.round(3).to_string())
print("\n=== nb composantes retenues (moyenne +/- std) ===")
for lvl in level_order:
    print(f"  {lvl:<12s} {ncomp_mean[lvl]:.1f} +/- {ncomp_std[lvl]:.1f}")
