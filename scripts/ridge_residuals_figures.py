...
for block in BLOCKS:
    for m in METRICS:
        ...
        fig.suptitle(f"{block} — {m}", fontsize=17)
        fig.tight_layout()
        fig.savefig(OUT / f"fig_residus_{block}_{m}.png", bbox_inches="tight")
        fig.savefig(OUT / f"fig_residus_{block}_{m}.pdf", bbox_inches="tight")
        plt.close(fig)

print("Figures ecrites dans", OUT)
for p in sorted(OUT.glob("fig_residus_*.png")):
    print("  ", p.name)
